...
无需安装，直接在浏览器中体验：<https://allfile-preview.softinstall.top/> —— 八类文件预览（图片 / 视频 / 音频 / PDF / TXT / Markdown / Word / Excel）、缩放、水印、本机文件选择均可交互体验

## 安装（npm 已发布）

```bash
# 核心包（原生 HTML / Vue2 / Vue3 直接使用自定义元素）
npm install @allfile-preview/core

# Vue3 封装（免 isCustomElement 配置，见下方"Vue3 方式 B"）
npm install @allfile-preview/vue3 @allfile-preview/core

# React 封装（支持 React 16.8+）
npm install @allfile-preview/react @allfile-preview/core
```

- 包页面：[@allfile-preview/core](https://www.npmjs.com/package/@allfile-preview/core) · [vue3](https://www.npmjs.com/package/@allfile-preview/vue3) · [react](https://www.npmjs.com/package/@allfile-preview/react)，各包内含完整使用文档
- UMD 免构建产物：`node_modules/@allfile-preview/core/dist/allfile-preview.umd.cjs`（用法见 [局域网（离线）部署](#局域网离线部署)）
- 以下所有接入示例均基于 npm 安装后的包名引入
...
```html
<script type="module">
  import '@allfile-preview/core' // 或 'node_modules/@allfile-preview/core/dist/index.js'
</script>
...
Vue2（`:actions` 绑定 JSON 字符串）：

```vue
<file-preview :src="url" :actions="actionsJson" @action="onAction" />
```

```js
// data: actionsJson: JSON.stringify([{ key: 'delete', label: '删除', danger: true }])
// methods:
onAction(e) {
  const { key, name } = e.detail
  if (key === 'delete') this.$axios.post('/api/delete', { name }) // 示例
}
```
...
Vue2（`:watermark` 绑定 JSON 字符串）/ Vue3 / React（对象直传）：

```vue
<file-preview :src="url" :watermark="watermarkJson" />

<script>
// Vue2：data 里 watermarkJson: JSON.stringify({ text: '内部资料', fontSize: 18 })
// Vue3 / React 包装组件：<FilePreview :watermark="{ text: '内部资料', fontSize: 18 }" />
</script>
```
...
实现方式：canvas 按配置绘制单个水印（含旋转包围盒）生成平铺图案，以 `background-repeat` 覆盖整个预览区，覆盖层 `pointer-events: none` 不影响缩放、拖拽、滚动等交互；配置变更时带竞态保护重新生成。注意水印为展示层防护，未做防篡改（用户可通过开发者工具移除），需要强防护的场景请在服务端处理。
...
file-preview {
  ...
  /* 工具栏/表面元素背景（meta 栏、sheet 标签栏、音频封面等，深浅内容区均保持统一工具栏风格） */
  --fv-surface: #1e293b;
...
- **core**：Lit 2 Web Components，Shadow DOM 样式隔离，属性全部为 string/number/boolean（Vue2 setAttribute 兼容硬约束）
- **构建**：Vite 4（Node 16 兼容）；core 双产物——ESM（`dist/index.js`，lit/pdfjs-dist/docx-preview/marked/xlsx 为 dependency）+ UMD（`dist/allfile-preview.umd.cjs`，全量内联约 2.1MB，全局变量 `FilePreview`）；vue3/react 双格式（ESM + CJS）
- **统一取数**：所有文档类组件（pdf/text/markdown/docx/xlsx）经 `utils/fetch-file` 统一 fetch，断网 / 跨域 / file:// / HTTP 状态码统一转中文提示
- **统一解码**：TXT 与 Markdown 共用 `utils/decode-text`（严格 UTF-8 → GB18030 → Big5 → Latin-1 回退链，含 BOM 处理）
- **PDF**：pdfjs-dist 3.11.174；worker 以 `?raw` 导入 + Blob URL 方式**完全内联**（ESM / UMD / file:// 均无需外部 worker 文件）；按 devicePixelRatio 高清渲染；翻页/缩放带竞态令牌防护；PasswordException 识别
- **Markdown**：marked 4.3.0（GFM + breaks）；输出直接注入 Shadow DOM 容器（**不做 sanitize**，须渲染可信内容）；竞态令牌防止快速切换 src 时旧内容覆盖新内容
- **TXT / MD 缩放**：CSS zoom 应用于正文容器；MD 适应宽度按 `min(排版最大宽, 容器宽)` 纯计算（不依赖运行时测量，向下取整到 1% 保证幂等），TXT 正文 pre-wrap 自适应、适应宽度即 100%
- **DOCX**：docx-preview + JSZip；`renderAsync` 的 styleContainer 指向 Shadow DOM 内部容器，渲染样式不泄漏到宿主页面
- **XLSX**：SheetJS（xlsx）`sheet_to_json` + `!merges`/`!cols`；合并单元格转 rowspan/colspan（带截断保护）；空表（无 `!ref`）安全跳过；缩放用 CSS zoom，模块加载时探针检测浏览器 zoom 对 getBoundingClientRect 的影响，保证"适应宽度"在不同 Chrome 版本下幂等
- **React 封装**：React ≤18 无法声明式绑定自定义元素事件，封装层通过 `ref + addEventListener` 桥接，回调经 ref 保持最新闭包
- **图片**：transform 矩阵（translate + scale + rotate）驱动缩放/旋转/拖拽；适应宽度按 `容器宽 / 自然宽 / fitScale` 纯计算（相对 contain 基准的倍数，重复点击幂等）；工具栏底部居中悬浮，按钮统一 32×32
- **水印**：`utils/watermark` 用 canvas 绘制单个水印（旋转后取包围盒 + 间距作为平铺单元）输出 data URL 平铺；图片以 `crossOrigin=anonymous` 加载（跨域未允许 CORS 时 canvas 被污染，`toDataURL` 抛异常后静默降级为不显示）；配置变更经竞态令牌防旧图覆盖新配置
- **播放器**：video/audio 共用 `<file-preview-player>`（内部组件），进度条拖动采用"拖动中仅更新 UI、松手才写 currentTime"防抖策略；`controls` 属性支持对象覆盖式 / 数组白名单两种配置形态，快退/快进步长可配
...
```html
<!-- 1. 把 dist/allfile-preview.umd.cjs 放到任意静态服务器（或内网共享目录 + 本地静态服务） -->
...
解析库（pdfjs-dist / docx-preview / marked / xlsx）已声明为依赖，安装时自动带入，构建器会正常打包。
...
不设置时嵌入字体的 PDF（绝大多数正式文档）仍可正常预览。本仓库 demo 中 `npm run gen:media` 会自动复制 cmaps 到 `demos/*/public/pdf/cmaps/`。
...
```bash
npm install            # 安装依赖
npm run gen:media      # 生成 demo 本地测试文件（WAV/SVG/TXT/PDF/MD/DOCX/XLSX + pdfjs cmaps，零二进制手写生成）
npm run build          # 构建三个包（core：ESM + UMD + d.ts；vue3/react：ESM + CJS）

npm run dev:vanilla    # 原生 HTML 案例  → http://127.0.0.1:5171
npm run dev:vue2       # Vue2 案例       → http://127.0.0.1:5172
```

包结构：`packages/core`（Lit 2 核心，四端通用）、`packages/vue3`（Vue3 封装，免 isCustomElement 配置）、`packages/react`（React 封装，支持 React 16.8+）；demo 提供 `demos/vanilla`（ESM + UMD 两种接入）与 `demos/vue2-demo` 两个工程，demo 的媒体源定义在 `demos/shared/media.ts`。
