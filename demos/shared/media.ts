 ...
 * 示例媒体资源（两个 demo 共用）。
 *
 * - 本地资源（media/...，相对路径）：由 `npm run gen:media` 生成到各 demo 的 public/media/，
 *   离线可用（音频为脚本生成的正弦波 WAV，图片为内置 SVG）；相对路径同时兼容
 *   本地 dev（页面位于 /）与 GitHub Pages 子路径（页面位于 /allfile-preview/）
 * - 远程资源：用于演示真实网络文件（需联网）
 */
export const MEDIA = {
  image: {
